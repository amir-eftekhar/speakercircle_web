 ...
 * 
 * This file contains utility functions for handling social media platforms,
 * including generating embed codes and extracting information from URLs.
 */

/**
 * Generates platform-specific embed code based on the URL
 */
export function generateEmbedCode(platform: string, url: string): string {
  ...
  if (platformLower.includes('instagram')) {
    ...
    if (match && match[1]) {
      ...
      return `<blockquote 
        ...
        style="background:#FFF; border:0; border-radius:3px; box-shadow:0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15); margin: 1px; max-width:100%; width:100%; padding:0;">
  ...
  if (platformLower.includes('youtube')) {
    const match = url.match(/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/([\w-]{11}))|youtu\.be\/([\w-]{11}))/);
  ...
  if (platformLower.includes('twitter') || platformLower.includes('x')) {
    return `<blockquote class="twitter-tweet" style="max-width:100%;">
      <a href="${url}"></a>
    </blockquote>
  ...
  if (platformLower.includes('facebook')) {
    ...
    <div class="fb-post" data-href="${url}" data-width="100%" style="max-width:100%;"></div>`;
  }
  ...
  // Default: just return a link if no specific embed code is available
...
/**
 * Gets the color associated with a social media platform
 */
export function getPlatformColor(platform: string): string {
  const platformLower = platform.toLowerCase();
  
  if (platformLower.includes('instagram')) return '#E1306C';
